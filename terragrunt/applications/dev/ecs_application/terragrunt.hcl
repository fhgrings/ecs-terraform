...
dependency "aws_alb" {
  ...
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    aws_alb_arn          = "arn:aws:elasticloadbalancing:us-east-1:643202173500:loadbalancer/app/ghost-alb/9XXX000XXX000XXX"
    aws_sg_egress_all_id = "some-id"
  }
}

dependency "efs" {
  config_path                             = "${get_parent_terragrunt_dir("root")}/applications/${include.stage.locals.stage}/efs_application"
  mock_outputs_allowed_terraform_commands = ["init", "validate", "plan"]
  mock_outputs = {
    aws_efs_id          = "arn:aws:elasticloadbalancing:us-east-1:643202173500:loadbalancer/app/ghost-alb/9XXX000XXX000XXX"
    aws_efs_access_point_id = "some-id"
  }
}
...
inputs = {
  ...
  ecs_volume = {
    file_system_id          = dependency.efs.outputs.aws_efs_id
    root_directory          = "/opt/data"
    transit_encryption      = "ENABLED"
    transit_encryption_port = 2999
    authorization_config = {
      access_point_id = dependency.efs.outputs.aws_efs_access_point_id
      iam             = "ENABLED"
    }
  }
